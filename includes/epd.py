...
class Epd(object):
  def __init__(self, spi, display):
    ...
    self.bus.write_byte_data(DEVICE_ADDRESS, 0x3a, 0x00) #SPI Passthrough
    #GPIO.setwarnings(False)
    #GPIO.setup(RST, GPIO.OUT)
    #GPIO.setup(DC, GPIO.OUT)
    #GPIO.setup(CS, GPIO.OUT)
    #GPIO.setup(BUSY, GPIO.IN)
    self.bus.write_byte_data(DEVICE_ADDRESS, 0x30, 0xfb) # RST_PIN = OUT, BUSY_PIN = IN
    self.bus.write_byte_data(DEVICE_ADDRESS, 0x31, 0xef) # DC_PI = OUT
  ...
  def _readBusy(self):
    for i in range(0, 400):
      if self.bus.read_byte_data(DEVICE_ADDRESS, 0x32) == 0xbf:
        # print 'Busy is Low'
        return
      time.sleep(0.01)
